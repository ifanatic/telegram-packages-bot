...
impl Botan {
    ...
    pub fn track<M: Encodable>(&self, uid: i64, name: &str, message: &M) -> Result<(), BotanError> {
    ...
    fn prepare_request<M: Encodable>(&self,
                                     uid: i64,
                                     name: &str,
                                     message: &M)
                                     -> Result<BotanRequest, BotanError> {
        let mut url = self.url.clone();
        let s_uid = &uid.to_string();
        let query_pairs: [(&str, &str); 3] =
            [("token", &self.token), ("uid", s_uid), ("name", name)];

        for query_pair in &query_pairs {
            url.query_pairs_mut().append_pair(query_pair.0, query_pair.1);
        }
